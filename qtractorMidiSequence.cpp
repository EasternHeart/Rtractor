...
// FIXME: No Comment
void qtractorMidiSequence::dumpEvents()
{
    QFile *qfTmpfile = new QFile("/tmp/Events_Dump.Rtractor.Debug");
    qfTmpfile->open(QFile::Truncate | QFile::WriteOnly);
    QTextStream qtsStream(qfTmpfile);
    qtractorMidiEvent *pEvent = m_events.first();
    for (; pEvent; pEvent = pEvent->next())
    {
        qtsStream << (int)(pEvent->pitchBend()) << ' '<< (int)(pEvent->note()) << ' ' << (int)(pEvent->velocity()) << endl;
    }
    qfTmpfile->close();
}
